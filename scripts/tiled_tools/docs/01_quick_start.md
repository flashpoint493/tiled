...
| 一张透明背景 tilesheet 里散排了多张贴图，想自动裁成独立 PNG | `tilesheet_split_connected` |
...
| 需要 Edge Set + Corner Set 两套地形 | `wang_2edge_corner_set` |
| Isometric 地图里要用 Terrain Brush 刷沙↔水 | `wang_2edge_set_iso_terrain` |
| Isometric Terrain 同时要 Edge + Corner | `wang_2edge_corner_set_iso_terrain` |
| 做一张完整 3×3 iso mock-up 大图 | `wang_2edge_big_iso` |
| 把编辑用 brush tileset 映射回运行时 tileset | `brush_variants_remap_tsx` → `remap_tmj_gids` |
