...
`pipelines/wang_2edge_set.yaml`，按注释提示切换即可。
...
会被 iso 化成菱形并标准化到 96×96 正方形画布**（菱形宽 96 高 48 贴底，上方留
...
└──┴──┴──┘                          │    │    │    │    │       上半 48px 透明给立绘 / 高物体
                                    │ ◇  │ ◇  │ ◇  │ ◇  │
4×4 平面 sheet                      └────┴────┴────┴────┘
（32×32 tile → 128×128）            4×4 iso sheet (默认 384×384)
```
...
```bash
# 默认 96×96 画布，菱形贴底
python -m tiled_tools run pipelines/wang_2edge_set_iso.yaml -v fg=sand.png -v bg=water.png

# 改成 128×128
python -m tiled_tools run pipelines/wang_2edge_set_iso.yaml \
...
**注意 2**：默认参数下 16 张 tile **完全同尺寸** 96×96，菱形贴底对齐，sheet
...
3. ▶ 运行
4. 右下产物：`sheet_xxxx/sheet.png` (384×384) + `sheet.tsx`
...
```text
wang_2edge_set_iso:
每格 96×96 透明画布，菱形贴底 —— 适合视觉 tileset / 物件垫底

wang_2edge_set_iso_terrain:
每格仍是 96×96 tileset 单元，方便在 Tileset 编辑器里框选/标记；
但内部 isometric grid 是 96×48 —— 适合 Tiled Isometric Terrain Brush
```
...
```bash
python -m tiled_tools run pipelines/wang_2edge_set_iso_terrain.yaml \
    -v fg=sand.png \
    -v bg=water.png \
    -v target=96 \
    -v half_target=48 \
...
默认产物是 `384×384` sheet（4×4，每格 `96×96`），但 terrain grid 是 `96×48`。

Tiled 中配套：


1. New Map：`Orientation=Isometric`，`Tile Width=96`，`Tile Height=48`
2. 打开生成的 `.tsx`（tileset 单元是 `96×96`，不是 `96×48`）
3. 确认 Tileset Properties 里 `Orientation=Isometric`、`Grid Width=96`、`Grid Height=48`
...
```bash
python -m tiled_tools run pipelines/wang_2edge_corner_set_iso_terrain.yaml \
    -v fg=sand.png \
    -v bg=water.png
```

默认产物是 `768×384` sheet（8×4，每格 `96×96`），内部 grid 仍是 `96×48`。打开 `.tsx` 后 Terrain Sets 面板里会同时有 Edge Set 和 Corner Set，不需要在透明 cell 上手工标边。
...
```bash
python -m tiled_tools run pipelines/wang_2edge_big_iso.yaml \
    -v fg=sand.png \
    -v bg=water.png \
    -v target=288 \
    -v half_target=144 \
...
- `flat_output`：未 iso 的 3×3 平面图（默认 `96×96`，当 `tile_size=32`）
- `output`：整体 iso 后的大图，默认放进 `288×288` 透明画布，下半是 `288×144` 的完整菱形图
...
完成后：
```
.web_runtime/outputs/sheet_xxxxxxxx/
